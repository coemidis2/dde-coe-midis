...
// ================= CIERRE FINAL EXPORTACION LIMPIA v45 - 07/05/2026 =================
(function(){
  const VERSION_CIERRE_LIMPIO = 'v45-exportacion-limpia-sin-internos';
  let pendiente = { dsId:'', tipo:'excel' };
  ...
  const val = (o, ...ks) => { for (const k of ks) { const v = o?.[k]; if (v !== undefined && v !== null && String(v).trim() !== '') return v; } return ''; };
  const fecha = (v) => {
    if (!v) return '';
    const s = String(v).slice(0,10);
    const d = new Date(`${s}T00:00:00`);
  ...
  const fechaHora = () => new Date().toLocaleString('es-PE', { hour12:false });
  const numeroDS = (d) => {
    const n = String(d?.numero || '').trim().padStart(3,'0');
    const a = String(d?.anio || new Date().getFullYear()).trim();
    return `${n}-${a}-PCM`.replace(/(-PCM)+$/i,'-PCM');
  };
  const tituloDS = (d) => `D.S. N°${numeroDS(d)}`;
  const nombreArchivo = (d, ext) => `DS_${numeroDS(d).replace(/[^0-9A-Za-zÁÉÍÓÚÑáéíóúñ-]/g,'_')}.${ext}`;
  const getDS = (id) => typeof buscarDecretoPorId === 'function' ? buscarDecretoPorId(id) : null;
  const accionesDS = (d) => {
    const lista = typeof cargarAccionesLocales === 'function' ? cargarAccionesLocales() : [];
    return lista.filter(a => String(val(a,'dsId','ds_id')) === String(d?.id));
  };
  const keyReunion = (n, f) => `${norm(n)}|${String(f || '').slice(0,10)}`;
  const reunionesConAcciones = (d) => {
    const mapa = new Map();
    accionesDS(d).forEach(a => {
      const n = val(a,'numeroReunion','numero_reunion');
      const f = val(a,'fechaReunion','fecha_reunion');
      if (!n || !f) return;
      const k = keyReunion(n, f);
      if (!mapa.has(k)) mapa.set(k, { key:k, numeroReunion:String(n), fechaReunion:String(f).slice(0,10) });
    });
    return [...mapa.values()].sort((a,b) => a.numeroReunion.localeCompare(b.numeroReunion, 'es') || a.fechaReunion.localeCompare(b.fechaReunion));
  };
  const accionesReunion = (d, r) => accionesDS(d).filter(a => keyReunion(val(a,'numeroReunion','numero_reunion'), val(a,'fechaReunion','fecha_reunion')) === r.key);
  ...
  const filaAccion = (a) => ({
    programa: val(a,'programaNacional','programa'),
    tipo: val(a,'tipoAccion','tipo'),
    codigo: val(a,'codigoAccion','codigo'),
    detalle: val(a,'detalle','accion','acciones'),
    unidad: val(a,'unidadMedida','unidad'),
    metaProgramada: val(a,'metaProgramada','meta_programada'),
    plazo: val(a,'plazoDias','plazo'),
    inicio: val(a,'fechaInicio','fecha_inicio'),
    fin: val(a,'fechaFinal','fecha_final'),
    metaEjecutada: val(a,'metaEjecutada','meta_ejecutada'),
    avance: val(a,'avance'),
    descripcion: val(a,'descripcionActividades','descripcion')
  });
  const seccionesConDatos = (d, r) => {
    const secciones = [
      { key:'preparacion', titulo:'ACCIONES DE PREPARACIÓN (para el caso de DEE por Peligro Inminente)', filas:[] },
      { key:'respuesta', titulo:'ACCIONES DE RESPUESTA', filas:[] },
      { key:'rehabilitacion', titulo:'ACCIONES DE REHABILITACIÓN', filas:[] },
    ...
    accionesReunion(d, r).map(filaAccion).forEach(f => {
      const sec = secciones.find(s => s.key === clasificar(f.tipo)) || secciones[3];
      sec.filas.push(f);
    });
    return secciones.filter(s => s.filas.length > 0);
  };
  function crearModal(){
    if (q('modalExportarReunionDSClean')) return;
    const div = document.createElement('div');
    div.id = 'modalExportarReunionDSClean';
    div.className = 'modal fade';
    div.tabIndex = -1;
    div.innerHTML = `
      <div class="modal-dialog modal-md modal-dialog-centered">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <div id="exportCleanInfo" class="alert alert-info py-2 small mb-3"></div>
            <label class="form-label">Número de reunión</label>
            <select id="exportCleanSelect" class="form-select"></select>
            <div class="form-text">Solo se muestran reuniones con acciones registradas. La reunión se usa únicamente para filtrar; no se imprimirá como bloque independiente.</div>
          </div>
          <div class="modal-footer">
            <button id="btnGenerarExportClean" type="button" class="btn btn-primary">Generar</button>
            <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Cancelar</button>
          </div>
        </div>
      </div>`;
    document.body.appendChild(div);
    q('btnGenerarExportClean')?.addEventListener('click', confirmar);
  }
  function abrir(id, tipo){
    const d = getDS(id);
    if (!d) return alert('Seleccione un Decreto Supremo válido para exportar.');
    const reuniones = reunionesConAcciones(d);
    if (!reuniones.length) return alert('El Decreto Supremo seleccionado no tiene acciones registradas para exportar.');
    crearModal();
    pendiente = { dsId:String(id), tipo };
    q('exportCleanInfo').innerHTML = `<strong>${esc(tituloDS(d))}</strong><br>Tiene ${reuniones.length} reunión${reuniones.length === 1 ? '' : 'es'} con acciones registradas.`;
    const sel = q('exportCleanSelect');
    sel.innerHTML = reuniones.map(r => `<option value="${esc(r.key)}">${esc(r.numeroReunion)} · ${esc(fecha(r.fechaReunion))}</option>`).join('');
    sel.dataset.reuniones = JSON.stringify(reuniones);
    q('btnGenerarExportClean').textContent = tipo === 'excel' ? 'Generar Excel' : 'Generar PDF';
    if (window.bootstrap?.Modal) bootstrap.Modal.getOrCreateInstance(q('modalExportarReunionDSClean')).show();
  }
  async function confirmar(){
    const d = getDS(pendiente.dsId);
    const sel = q('exportCleanSelect');
    if (!d || !sel) return;
    let reuniones = [];
    try { reuniones = JSON.parse(sel.dataset.reuniones || '[]'); } catch {}
    const r = reuniones.find(x => x.key === sel.value);
    if (!r) return alert('Seleccione una reunión válida.');
    const secciones = seccionesConDatos(d, r);
    if (!secciones.length) return alert('La reunión seleccionada no tiene acciones registradas para exportar.');
    if (window.bootstrap?.Modal) bootstrap.Modal.getOrCreateInstance(q('modalExportarReunionDSClean')).hide();
    if (pendiente.tipo === 'excel') await generarExcel(d, r, secciones);
    else generarPDF(d, r, secciones);
  }
  ...
  async function generarExcel(d, r, secciones){
    if (!window.ExcelJS) return alert('No se cargó ExcelJS. Revise conexión a internet o CDN.');
    const wb = new ExcelJS.Workbook();
    wb.creator = 'DEE MIDIS'; wb.created = new Date();
    const ws = wb.addWorksheet(`DS_${numeroDS(d)}`.replace(/[\\/*?:\[\]]/g,'_').slice(0,31));
    ws.pageSetup = { paperSize:9, orientation:'landscape', fitToPage:true, fitToWidth:1, fitToHeight:0, horizontalCentered:true };
    ws.pageMargins = { left:0.25, right:0.25, top:0.35, bottom:0.35, header:0.15, footer:0.15 };
    ws.columns = [{width:24},{width:26},{width:18},{width:60},{width:16},{width:14},{width:12},{width:16},{width:16},{width:14},{width:12},{width:44}];
    const azul = 'FF1F4E79';
    const titleCell = (row, text, size=12) => { ws.mergeCells(`A${row}:L${row}`); const c = ws.getCell(`A${row}`); c.value = text; c.font = { bold:true, size, color:{argb:azul} }; c.alignment = { horizontal:'center', vertical:'middle', wrapText:true }; };
    let row = 1;
    titleCell(row++, 'MATRIZ EJECUTIVA DE SEGUIMIENTO DE ACCIONES EN LA DECLARATORIA DE ESTADO DE EMERGENCIA');
    titleCell(row++, tituloDS(d), 11);
    titleCell(row++, 'SECTOR: MINISTERIO DE DESARROLLO E INCLUSIÓN SOCIAL', 10);
    titleCell(row++, `FECHA DE REPORTE: ${fecha(new Date().toISOString())}`, 10);
    row++;
    const motivos = val(d,'motivos','exposicion_motivos');
    if (motivos) {
      ws.mergeCells(`A${row}:L${row}`); ws.getCell(`A${row}`).value = 'ACCIONES A REALIZAR POR EL SECTOR SEGÚN LA EXPOSICIÓN DE MOTIVOS';
      ws.getRow(row).eachCell(c => { c.font = {bold:true, color:{argb:'FFFFFFFF'}}; c.fill = {type:'pattern', pattern:'solid', fgColor:{argb:azul}}; c.alignment = {horizontal:'center', vertical:'middle', wrapText:true}; c.border = {top:{style:'thin'},left:{style:'thin'},bottom:{style:'thin'},right:{style:'thin'}}; });
      row++;
      ws.mergeCells(`A${row}:L${row}`); ws.getCell(`A${row}`).value = motivos; ws.getRow(row).height = 42;
      ws.getCell(`A${row}`).alignment = {wrapText:true, vertical:'top'}; ws.getCell(`A${row}`).border = {top:{style:'thin'},left:{style:'thin'},bottom:{style:'thin'},right:{style:'thin'}};
      row += 2;
    }
    const styleHeader = (rnum) => ws.getRow(rnum).eachCell(c => { c.font = {bold:true, color:{argb:'FFFFFFFF'}, size:9}; c.fill = {type:'pattern', pattern:'solid', fgColor:{argb:azul}}; c.alignment = {horizontal:'center', vertical:'middle', wrapText:true}; c.border = {top:{style:'thin'},left:{style:'thin'},bottom:{style:'thin'},right:{style:'thin'}}; });
    const styleRow = (rnum) => ws.getRow(rnum).eachCell(c => { c.alignment = {vertical:'top', wrapText:true}; c.border = {top:{style:'thin'},left:{style:'thin'},bottom:{style:'thin'},right:{style:'thin'}}; });
    secciones.forEach(sec => {
      ws.mergeCells(`A${row}:L${row}`); ws.getCell(`A${row}`).value = sec.titulo; styleHeader(row); row++;
      ws.getRow(row).values = headers; styleHeader(row); row++;
      sec.filas.forEach(f => {
        ws.getRow(row).values = [f.programa, f.tipo, f.codigo, f.detalle, f.unidad, f.metaProgramada, f.plazo, fecha(f.inicio), fecha(f.fin), f.metaEjecutada, f.avance, f.descripcion];
        styleRow(row); row++;
      });
      row++;
    });
  ...
  function generarPDF(d, r, secciones){
    if (!window.jspdf?.jsPDF) return alert('No se cargó jsPDF. Revise conexión a internet o CDN.');
    const { jsPDF } = window.jspdf;
    const doc = new jsPDF({ orientation:'landscape', unit:'mm', format:'a4' });
    const azul = [31,78,121];
    doc.setFont('helvetica','bold'); doc.setTextColor(...azul); doc.setFontSize(12);
    doc.text('MATRIZ EJECUTIVA DE SEGUIMIENTO DE ACCIONES EN LA DECLARATORIA DE ESTADO DE EMERGENCIA', 148.5, 12, {align:'center'});
    doc.setFontSize(11); doc.text(tituloDS(d), 148.5, 19, {align:'center'});
    doc.setFontSize(9); doc.setTextColor(0,0,0); doc.text('SECTOR: MINISTERIO DE DESARROLLO E INCLUSIÓN SOCIAL', 148.5, 26, {align:'center'});
    doc.text(`FECHA DE REPORTE: ${fecha(new Date().toISOString())}`, 148.5, 32, {align:'center'});
    let y = 39;
    const motivos = val(d,'motivos','exposicion_motivos');
    if (motivos) {
      doc.autoTable({ startY:y, body:[['ACCIONES A REALIZAR POR EL SECTOR SEGÚN LA EXPOSICIÓN DE MOTIVOS', motivos]], theme:'grid', styles:{fontSize:7, cellPadding:1.2, overflow:'linebreak', valign:'top'}, columnStyles:{0:{fontStyle:'bold', fillColor:[221,235,247], cellWidth:72},1:{cellWidth:202}}, margin:{left:11,right:11} });
    ...
    secciones.forEach(sec => {
      if (y > 175) { doc.addPage(); y = 12; }
      doc.setFont('helvetica','bold'); doc.setFontSize(8); doc.setTextColor(...azul); doc.text(sec.titulo, 8, y); y += 2;
      const body = sec.filas.map(f => [f.programa, f.tipo, f.codigo, f.detalle, f.unidad, f.metaProgramada, f.plazo, fecha(f.inicio), fecha(f.fin), f.metaEjecutada, f.avance, f.descripcion]);
      doc.autoTable({ startY:y, head, body, theme:'grid', headStyles:{fillColor:azul, textColor:[255,255,255], halign:'center', valign:'middle', fontSize:5.7}, styles:{fontSize:5.3, cellPadding:0.65, overflow:'linebreak', valign:'top', lineColor:[90,90,90], lineWidth:0.1}, columnStyles:{0:{cellWidth:23},1:{cellWidth:28},2:{cellWidth:17},3:{cellWidth:58},4:{cellWidth:16},5:{cellWidth:13},6:{cellWidth:11},7:{cellWidth:16},8:{cellWidth:16},9:{cellWidth:13},10:{cellWidth:11},11:{cellWidth:45}}, margin:{left:6,right:6}, didDrawPage:()=>{doc.setFontSize(6); doc.setTextColor(100); doc.text(`Exportado desde DEE MIDIS · ${fechaHora()}`, 8, 204);} });
      y = doc.lastAutoTable.finalY + 5;
    });
    doc.save(nombreArchivo(d,'pdf'));
  }
  window.exportarDSExcel = (id) => abrir(id, 'excel');
  window.exportarDSPDF = (id) => abrir(id, 'pdf');
  try { exportarDSExcel = window.exportarDSExcel; exportarDSPDF = window.exportarDSPDF; } catch {}
  window.addEventListener('click', function(e){
    ...
    const dsId = q('exportDs')?.value || q('accionDs')?.value || '';
    if (!dsId) return alert('Seleccione un Decreto Supremo para exportar.');
    abrir(dsId, excelBtn ? 'excel' : 'pdf');
  }, true);
  document.addEventListener('DOMContentLoaded', () => setTimeout(() => { crearModal(); console.info('DEE MIDIS cierre aplicado:', VERSION_CIERRE_LIMPIO); }, 500));
})();
