...
function normalizeRole(value) {
  ...
  const clean = raw.normalize('NFD').replace(/[\u0300-\u036f]/g, '').trim().toUpperCase();
  if (clean === 'CONSULTAR' || clean === 'CONSULTA') return 'Consulta';
  if (clean === 'ADMINISTRADOR') return 'Administrador';
  if (clean === 'REGISTRADOR') return 'Registrador';
  return raw;
}
...
export async function getSessionFromRequest(context) {
  ...
  if (!token) {
    ...
    const localRole = normalizeRole(context.request.headers.get('x-dee-user-role') || '');
  ...
  const normalizedRole = normalizeRole(user.role === 'Evaluador' ? 'Revisor' : user.role);
...
export async function requireSession(context, allowedRoles = []) {
  ...
  if (allowedRoles.length) {
    ...
    if (!normalizedAllowed.includes(normalizeRole(session.role))) {
...
export function verifyCsrf(request) {
  ...
  if (request.headers.get('x-dee-local-session') === '1' &&
      normalizeRole(request.headers.get('x-dee-user-role') || '') === 'Administrador') {
    return true;
  }
