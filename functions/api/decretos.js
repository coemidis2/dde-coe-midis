...
export async function onRequestPut(context) {
  ...
  try {
    ...
    const current = await context.env.DB
      .prepare('SELECT id, numero, version, locked, deleted_at FROM decretos WHERE id = ?')
      .bind(body.id)
      .first();
    ...
    await writeAudit(context.env, {
      actor: auth.session.email,
      action: 'update_decreto',
      detail: body.numero || body.id,
      entity_type: 'decreto',
      entity_id: body.id
    });
...
export async function onRequestDelete(context) {
  ...
  try {
    ...
    await writeAudit(context.env, {
      actor: auth.session.email,
      action: 'delete_decreto',
      detail: current.numero || id,
      entity_type: 'decreto',
      entity_id: id
    });
