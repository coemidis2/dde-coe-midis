...
function normalizeRoleInput(role, programa) {
  ...
  // Acepta las variantes que el front ha usado en distintas versiones:
  // Consulta / Consultar, Registrador|Programa y Registrador - Programa.
  const roleForMatch = normalizeText(rawRole);

  if (roleForMatch === 'CONSULTAR' || roleForMatch === 'CONSULTA') {
    return { role: 'Consulta', programa: '' };
  }

  if (roleForMatch === 'ADMINISTRADOR') {
    return { role: 'Administrador', programa: '' };
  }

  if (rawRole.includes('|')) {
    const parts = rawRole.split('|');
    if (normalizeText(parts[0]) === 'REGISTRADOR') {
      return { role: 'Registrador', programa: normalizeProgramName(parts.slice(1).join('|') || rawPrograma) };
    }
  }

  const dashMatch = rawRole.match(/^Registrador\s*[-–—]\s*(.+)$/i);
  if (dashMatch) {
    return { role: 'Registrador', programa: normalizeProgramName(dashMatch[1] || rawPrograma) };
  }

  if (roleForMatch === 'REGISTRADOR') {
    return { role: 'Registrador', programa: normalizeProgramName(rawPrograma) };
  }

  return { role: rawRole, programa: normalizeProgramName(rawPrograma) };
}

function isValidRole(role) {
  return ['Administrador', 'Registrador', 'Consulta', 'Consultar'].includes(role);
}
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    const temporaryPassword = String(body.password || body.temporaryPassword || body.clave || '').trim() || generateTemporaryPassword();
